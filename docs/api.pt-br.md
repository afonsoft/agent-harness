...
A página `/workflow` é um monitor read-only de GitHub Actions: `GET .../workflows` → `200 { workflows: [{ id, name, path, state, htmlUrl, lastRun }], recentRuns: [{ ...run, workflowId }], degraded }` — os badges de último run vêm de uma única chamada repo-level de runs agrupada por `workflowId` (2 chamadas GitHub no total, sem N+1); `recentRuns` traz os 5 runs mais recentes do repo para a strip "Latest runs". Um deadline server-side (`Taskboard:GitHub:WorkflowsDeadlineSeconds`, padrão 25s) disputa a chamada — em timeout/falha o endpoint retorna `200` com `degraded: true` e workflows sem badge em vez de erro. `GET .../workflows/{id}/runs?take=10` → `200 { runs: [{ id, name, displayTitle, runNumber, event, status, conclusion, headBranch, headSha, actorLogin, createdAt, updatedAt, runStartedAt, htmlUrl, workflowId }] }`. Repo desconhecido → `404 { "error": "repo-not-found" }`. Os badges mapeiam `success`→verde, `failure`/`timed_out`/`startup_failure`/`action_required`→vermelho, `in_progress`/`queued`/`requested`/`waiting`/`pending`→âmbar pulsante (dirige o auto-refresh de 60s), demais→cinza; falhas exibem alerta com Retry e mantêm a última lista válida.
...
`POST` com body `{ runId, repositoryPath, baseBranch?, taskSlug, retainOnFailure? }` → `201` com `{ worktreeId, runId, path, branch, status, repositoryPath, baseBranch, commitSha?, retainOnFailure, createdAt, updatedAt, version }`. Cria um `git worktree` dedicado em `{worktreeRoot}/{runId}` — `Taskboard:WorktreeRoot` (com expansão de `~`), default `~/repos` para os worktrees ficarem ao lado dos clones no workspace — numa nova branch `feature/agent-{runId}-{slug}` (sufixo incremental em colisão); `baseBranch` usa `main` como padrão na API e `HEAD` quando o orquestrador isola um run. Idempotente por `runId` — sessão ativa é reutilizada. Um diretório existente no destino só é limpo quando é worktree stale (`.git` arquivo) ou vazio — qualquer outro conteúdo (ex.: um clone real) falha em vez de ser apagado.
...
`GET templates` → `200` com os quatro templates DAG embutidos: `standard-feature` (architect → approval-gate → builder → verifier → reviewer), `quick-patch` (builder → verifier), `test-driven` (architect → tester → builder → verifier) e `single-agent` (builder → verifier). Cada template também retorna `stages: [{ key, name, kind, defaultAgent?, defaultTier?, dependsOn }]` para a UI renderizar seletores por estágio.

`POST start` body `{ templateId, repositoryFullName, repositoryPath, baseBranch, issueId?, initialPrompt, maxBudgetUsd?, agentOverride?, tierOverride?, skipVerification?, stageOverrides?, singleAgent?, singleAgentType?, singleAgentTier? }` → `201` com `{ pipelineExecutionId, templateId, status, worktreePath?, repositoryPath?, failureReason?, stages: [{ stageKey, name, kind, status, role?, agent?, attempts, handoffSummary?, lastError?, dependsOn, triedAgents?, autoRetryCount?, nextAutoRetryAtUtc? }] }`. Template desconhecido ou grafo cíclico → `400 INVALID_PIPELINE_DAG`. `agentOverride`/`tierOverride`/`skipVerification` são exclusivos de `single-agent` — substituem o agente/tier do estágio AgentWork e removem o estágio Verification; enviá-los com outro template → `400`.

SPEC-20260923-cockpit-run-hardening RF-001: `repositoryPath` no body é ignorado — o clone é provisionado no servidor pelo `IRepositoryProvisioningService` como `<workspace root>/<repo-name>` (`git clone` quando ausente/vazio; um diretório existente só é reutilizado quando é work tree git cujo `origin` bate com `owner/name`; repos estrangeiros e dirs não-repo → `422 REPOSITORY_PROVISIONING_FAILED`, nunca apagados ou reutilizados silenciosamente). O `repositoryPath` retornado é o clone provisionado e o worktree do run é sempre criado a partir dele — o workspace root nunca é o caminho do repositório.

`stageOverrides` é um mapa `stageKey → { agent?, tier? }` válido para os estágios AgentWork de qualquer template (chaves desconhecidas, override em estágio não-agente ou agente+tier `null` → `400`). `singleAgent: true` vincula um único CLI a todos os estágios AgentWork — `singleAgentType` o escolhe explicitamente, `null` significa Auto (primeiro elegível). No start, cada estágio AgentWork resolve seu CLI contra o conjunto elegível (instalado + autenticado + habilitado via `IAgentEligibilityService`): default do template quando elegível, senão Auto; um CLI explicitamente escolhido mas inelegível → `422 AGENT_NOT_ELIGIBLE`, e nenhum CLI elegível → `422` também.
...
`POST .../approve` body `{ comment? }` libera um gate `WaitingApproval` e despacha os dependentes → `200`. `POST .../retry` body `{ adjustedPrompt? }` reenfileira um estágio `Failed` sem reiniciar o pipeline → `202` (limpa `triedAgents` para a cadeia de fallback completa ficar disponível de novo). `POST .../cancel` encerra os processos em voo e transiciona o pipeline para `Cancelled` → `200`. Aprovar/retentar estágio em estado incompatível → `400`; pipeline desconhecido → `404`.

O despacho roda no timer do `PipelineEngineService` mais kicks síncronos após `start`/`approve`/`retry`; o `PipelineEngine` resolve o DAG por escopo de execução para que estágios paralelos nunca compartilhem um `DbContext`. Cada despacho de AgentWork executa o CLI vinculado ao estágio; quando o modelo emitido é rejeitado (`unrecognized_model`), o engine retenta uma vez no mesmo CLI sem model flag. Uso/custo são atribuídos ao CLI que de fato executou cada tentativa e toda tentativa faturada emite um evento `metric`.

A SPEC-20260923 RF-002 substitui o fallback no mesmo tick: um estágio falho numa execução `AwaitingRetry` é retentado automaticamente numa agenda persistida (`autoRetryCount`/`nextAutoRetryAtUtc` sobrevivem a restarts) — `Taskboard:Pipelines:AutoRetry` (`Enabled` default `true`, `AttemptsPerAgent` default 5, `Interval` default `00:01:00`). Ao esgotar as tentativas do CLI vinculado, a varredura rotaciona para o próximo CLI elegível fora de `triedAgents` e repete o mesmo budget; um CLI vinculado que virou inelegível rotaciona imediatamente. Quando não resta CLI elegível não tentado, o run termina em `Failed` com `failureReason` (stage key, total de tentativas, agents tentados, último erro) — estágios pendentes/em espera viram `Skipped` e nada mais é despachado. Agendamento, despacho, rotação e a falha terminal são emitidos como eventos `stage`/`error`/`run_status`; a página do run mostra o `failureReason` num alerta danger.
...
"Runs" do cockpit são execuções de pipeline. `GET runs` → `200` com as execuções recentes (mais novas primeiro, mesmo shape de `GET /api/harness/pipelines/{id}`). `POST runs` body `{ templateId, repositoryFullName, baseBranch, issueId?, specPath?, prompt, maxBudgetUsd?, agentOverride?, tierOverride?, skipVerification?, stageOverrides?, singleAgent?, singleAgentType?, singleAgentTier? }` não envia caminho de repositório — o clone é provisionado no servidor pelo `IRepositoryProvisioningService` (`~/repos/<name>`, confinado ao workspace root, dirs estrangeiros/não-repo → `422`), anexa o conteúdo de `specPath` ao prompt quando legível e inicia o pipeline → `201`. Os overrides seguem as mesmas regras do `POST pipelines/start` — `singleAgent`/`stageOverrides` valem para qualquer template, `agentOverride`/`tierOverride`/`skipVerification` continuam exclusivos de `single-agent` — e escolhas explícitas inelegíveis retornam `422`. Com `issueId` presente, um `IssueHistoryEvent` `pipeline-run` vincula a issue ao id do run (aba Histórico da issue → "Open in Cockpit"). `GET {id}` → `200 { execution, telemetry?, worktree? }` ou `404`.

`GET {id}/events` → `200 { events, nextAfter, hasMore }` (SPEC-20260923 RF-004): `events` são `CockpitEventDto` (`{ runId, timestampUtc, kind, title, payloadJson? }`) lidos do stream de eventos normalizado durável (`IAgentExecutionEventSink` — sobrevive a restarts), mesclados com chunks `agent_output` live-only ainda no buffer volátil. `after`/`take` paginam por sequence (`after=nextAfter` para a próxima página). Os kinds normalizados `output`/`lifecycle`/`verification`/`error`/`approval`/`steer`/`metric` são mapeados de volta aos kinds do cockpit para replay — os eventos também são transmitidos ao vivo pelo hub SignalR abaixo.
...
`POST {id}/approvals/{requestId}` body `{ action, comment? }` — `requestId` usa a convenção `stage:<stageKey>` publicada pelo `RequireApproval`; `Allow` aprova o estágio `WaitingApproval`, `Deny` rejeita com o comentário como motivo e encerra o run em `Failed` terminal → `200`. Run/request desconhecido → `404`.
...
Hub SignalR `/harness-cockpit-hub` (autenticado): client → server `JoinRunGroup(runId)` / `LeaveRunGroup(runId)` / `JoinRunsGroup` / `LeaveRunsGroup` — o grupo compartilhado `runs` carrega os eventos de todos os runs para a lista atualizar ao vivo; server → client `ReceiveCockpitEvent(CockpitEventDto)` e `RequireApproval(ApprovalRequestDto)` (`{ runId, requestId, title, description, options, summary? }` — `summary` é um digest de uma linha repo · run · stage · handoff usado no toast/notificação do browser). Toda transição no nível da execução publica um evento `run_status` (payload `{ status, completedAtUtc? }`) no grupo do run e em `runs`; as linhas da lista mostram badge `Needs approval` enquanto um gate aguarda. O push de aprovação aparece como toast in-app mais Notification do browser (quando permitido) antes do modal abrir. As páginas Blazor são `/cockpit` (lista + start, refresh de status ao vivo + contagem de aprovações pendentes, reconciliação na reconexão) e `/cockpit/runs/{id}` (timeline ao vivo, aba Logs com follow-scroll + filtros de stage/stream + copiar/baixar, aba **Shell** interativa — `TerminalHub.OpenForRun(runId)` abre um PTY com cwd no worktree do run resolvido no servidor e confinado ao workspace root — aba Arquivos explorer, aba Diff colapsável por arquivo, barra de steer, modal de aprovação, header FinOps).
