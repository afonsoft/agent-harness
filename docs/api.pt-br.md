...
O despacho roda no timer do `PipelineEngineService` mais kicks síncronos após `start`/`approve`/`retry`; o `PipelineEngine` resolve o DAG por escopo de execução para que estágios paralelos nunca compartilhem um `DbContext`. Cada despacho de AgentWork re-checa elegibilidade — um CLI desabilitado após o start é pulado, nunca invocado — e em caso de falha faz fallback para o próximo CLI elegível ainda não tentado, reutilizando o mesmo prompt/contexto sintetizado (`triedAgents` registra a cadeia, `attempts` conta os despachos). Quando o modelo emitido é rejeitado (`unrecognized_model`), o engine retenta uma vez no mesmo CLI sem model flag antes de consumir o próximo candidato; uso/custo são atribuídos ao CLI que de fato executou cada tentativa, e as transições de fallback aparecem como eventos `stage` na timeline do cockpit.
