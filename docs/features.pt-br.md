...
- UI de chat completa em `/ai-chat` (SPEC-20260922-ai-chat-command-bar): área de mensagens com renderização markdown, composer, indicador "digitando" e auto-scroll — sem sidebar; todo o histórico de threads fica atrás de um ícone Threads ao lado do título (selecionar + excluir com confirmação inline), com um ícone New ao lado
- Command bar acima do composer configura a próxima conversa: Mode (Assistant/Agent), picker de Agent CLI (só CLIs elegíveis, exibe o nome), popup de repo (texto livre ou o combobox de repositórios), workspace fixo `~/repos`, popup de Model (tier Lite/Normal/Ultra ou modelo explícito do catálogo) e popup de Sandbox (só no modo Agent); o primeiro Send cria a thread — o título deriva automaticamente do prompt (~60 chars) — e trava a barra até New destravar; quick-switch de modo/modelo continua usável numa sessão de agente viva
- Erros mostram o motivo real do backend — `detail`/`code` do `problem+json` e envelopes `{ error.message }` chegam ao toast em vez de falha genérica; deletar uma thread stale é idempotente (204) e o GET de thread única retorna 404 `THREAD_NOT_FOUND` para ids desconhecidos
- O stream SSE por thread emite um comentário heartbeat `: hb` a cada `Taskboard:AiChat:SseHeartbeatSeconds` (default 15s) para conexões ociosas sobreviverem a reverse proxies, e fecha limpo quando o cliente desconecta; threads desconhecidas falham rápido com 404 em vez de ficar penduradas
- Os caminhos de fechamento de modal tiram o foco do elemento ativo antes do `aria-hidden` (`taskboard.blurActiveElement`) — sem warning de focus-trap para tecnologia assistiva
...
- Página `/terminal`: bash PTYs interativos via SignalR (`/terminal-hub`) com xterm.js — **múltiplas sessões em abas** (até 8 por usuário, `Open()` retorna `sessionId` roteado na mesma conexão), timeout de 30 min ocioso por aba, fechar/reabrir por aba, `?cmd=` pré-preenche a primeira aba; abas novas abrem no workdir do repositório selecionado (`~/repos/<name>`, caindo para a raiz do workspace); teclas de terminal nativo: Ctrl+C copia a seleção (ou envia SIGINT), Ctrl+V / Ctrl+Shift+V / Shift+Insert colam, flag `Taskboard:Terminal:Enabled`; **modo focus** (SPEC-20260923-terminal-focus-mode): um botão expandir/restaurar transforma a página num overlay de viewport inteiro — topbar, título e descrição somem, a tab strip compacta permanece para trocar de aba e um botão flutuante de restaurar é a saída (CSS-only via `html[data-terminal-focus]`, estado só de sessão, `Esc` continua indo ao PTY); **keybar virtual** (SPEC-20260923-terminal-virtual-keybar): no modo focus em dispositivos touch (`pointer: coarse`), uma barra inferior oferece `Esc`, `Tab`, `Shift+Tab`, setas, `Home`/`End`/`PgUp`/`PgDn`, um `Ctrl` sticky de um toque (a próxima letra vira seu byte de controle — `Ctrl`→`C` envia SIGINT, `Ctrl`→`V` cola) e uma tecla **Paste** dedicada (`navigator.clipboard.readText` → paste do xterm, com hint no terminal quando a API está indisponível)
...
- `/specs` cataloga os `.specs/SPEC-*.md` do clone do repositório selecionado (parser Markdig — metadata, RFs, ACs BDD, progresso de tasks, avisos de drift) com filtros de status (All/Draft/Approved/InImplementation/Done/Deprecated) e busca por id/título
- Visão default (filtro All, sem busca) mostra todas as specs não-Done — `Deprecated` incluído — mais apenas as últimas 10 `Done` (id desc), com expansor "Show all N Done specs" quando truncado; qualquer filtro ou busca ignora o cap e mostra todos os resultados
