...
- `/terminal` page: interactive bash PTYs over SignalR (`/terminal-hub`) with xterm.js — **multiple tabbed sessions** (up to 8 per user, `Open()` returns a `sessionId` routed over one connection), per-tab 30 min idle timeout, per-tab close/reopen, `?cmd=` pre-fills the first tab; new tabs open in the selected repository's workdir (`~/repos/<name>`, falling back to the workspace root); native terminal keys: Ctrl+C copies the selection (or sends SIGINT), Ctrl+V / Ctrl+Shift+V / Shift+Insert paste, `Taskboard:Terminal:Enabled` flag; **focus mode** (SPEC-20260923-terminal-focus-mode): an expand/restore toggle turns the page into a full-viewport overlay — topbar, page title and description hide, a compact tab strip stays for switching, and a floating restore button is the way out (CSS-only via `html[data-terminal-focus]`, session-only state, `Esc` still reaches the PTY); **virtual keybar** (SPEC-20260923-terminal-virtual-keybar): in focus mode on touch devices (`pointer: coarse`), a bottom toolbar offers `Esc`, `Tab`, `Shift+Tab`, arrows, `Home`/`End`/`PgUp`/`PgDn`, a one-shot sticky `Ctrl` (next letter becomes its control byte — `Ctrl`→`C` sends SIGINT, `Ctrl`→`V` pastes) and a dedicated **Paste** key (`navigator.clipboard.readText` → xterm paste, with an in-terminal hint when the API is unavailable)
...
- **Cockpit (HITL)**: `/cockpit` lists pipeline runs and starts new ones (template, repo — prefilled from the global selector — branch, budget cap). The New Run dialog works for every template: a **Single agent** checkbox binds one CLI (Auto or picked from the available CLIs) plus a tier to all AgentWork stages, and a collapsible **Options** section exposes per-stage Agent CLI (`Auto` first, then only installed+authenticated+enabled CLIs) and tier (Lite/Normal/Ultra) selects; when no CLI is available the dialog warns and Start is disabled. Each stage card shows the effective CLI and `×N` attempt count, with the fallback chain (`tried: A → B`) on hover. `/cockpit/runs/{id}` streams a structured live timeline (stage / agent_output / verification / approval / steer / tool_call cards) over the `/harness-cockpit-hub` SignalR group with REST replay on reconnect, a read-only xterm.js **Logs** tab that renders the run's `agent_output` stream in real time with follow-scroll ("↓ novos logs" pill), stage/stream filters, copy/download/clear controls and a live badge, an **Arquivos** explorer tab that browses the run's worktree (`GET …/worktrees/{runId}/files` + `…/files/content` — lazy tree, read-only, `.git` hidden, path-jailed, 512 KB cap), a **Diff** tab that lists changed files as collapsible per-file cards (`GET …/worktrees/{runId}/diff`, `+/-` per file via `--numstat`, truncated above 2 MB), a steer bar (`POST …/runs/{id}/steer` — queued into the next stage prompt), pause/resume controls (`POST …/runs/{id}/pause`/`resume` — freezes the DAG between stages, in-flight stage completes), approval gates (`POST …/approvals/{requestId}`, `stage:<key>` convention), a stop button and a one-click `create-pr` that commits + pushes the worktree, opens the GitHub PR and moves the bound issue to `in_review` with the PR link commented. The header shows live FinOps metrics (tokens in/out, USD cost, elapsed time). SPEC-20260923 hardening: run start provisions the clone server-side under `~/repos/<name>` (foreign/non-repo dirs → `422`), the worktree is always cut from that clone and the agent CLI runs inside it; failed stages auto-retry on a persisted schedule (5 attempts × 1 min per CLI, then rotation to the next untried eligible CLI, then terminal `Failed` with a detailed `failureReason`); the list and detail pages update in real time via the shared `runs` SignalR group (`run_status` on every transition, `Needs approval` badges, reconnect reconciliation — no manual F5); approval gates push a summary toast + browser Notification + modal; a **Shell** tab opens an interactive PTY rooted at the run worktree; and `GET …/runs/{id}/events` serves the durable normalized event stream (`{ events, nextAfter, hasMore }`), not just the volatile buffer.
...
- `/specs` catalogs `.specs/SPEC-*.md` of the selected repo clone (Markdig parser — metadata, RFs, BDD ACs, task progress, drift warnings) with status filters (All/Draft/Approved/InImplementation/Done/Deprecated) and search by id/title
- Default view (All filter, empty query) shows every non-Done spec — `Deprecated` included — plus only the latest 10 `Done` (spec id desc), with a "Show all N Done specs" expander when truncated; any filter or search bypasses the cap and shows every match
