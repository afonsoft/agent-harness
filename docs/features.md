...
- **Cockpit (HITL)**: `/cockpit` lists pipeline runs and starts new ones (template, repo — prefilled from the global selector — branch, budget cap). With the `single-agent` template the dialog adds an Agent CLI combo (available agents only), a model-tier select and a "run verification stage" checkbox. `/cockpit/runs/{id}` streams a structured live timeline (stage / agent_output / verification / approval / steer / tool_call cards) over the `/harness-cockpit-hub` SignalR group with REST replay on reconnect, a read-only xterm.js **Terminal** tab that renders the run's `agent_output` stream (replayed from the buffered events on reload), a unified Git diff tab (`GET …/worktrees/{runId}/diff`, truncated above 2 MB), a steer bar (`POST …/runs/{id}/steer` — queued into the next stage prompt), pause/resume controls (`POST …/runs/{id}/pause`/`resume` — freezes the DAG between stages, in-flight stage completes), approval gates (`POST …/approvals/{requestId}`, `stage:<key>` convention), a stop button and a one-click `create-pr` that commits + pushes the worktree and opens the GitHub PR. The header shows live FinOps metrics (tokens in/out, USD cost, elapsed time).
