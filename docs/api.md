...
Dispatch runs on the `PipelineEngineService` background timer plus synchronous kicks after `start`/`approve`/`retry`; `PipelineEngine` resolves the DAG per execution scope so parallel stages never share a `DbContext`. Each AgentWork dispatch re-checks eligibility — a CLI disabled after start is skipped, never invoked — and on failure falls back to the next eligible CLI not yet tried, reusing the same synthesized prompt/context (`triedAgents` records the chain, `attempts` counts dispatches). When the emitted model is rejected (`unrecognized_model`) the engine retries once on the same CLI without a model flag before consuming the next candidate; usage/cost are attributed to the CLI that actually ran each attempt, and fallback transitions appear as `stage` events in the cockpit timeline.
