...
The `/workflow` page is a read-only GitHub Actions monitor: `GET .../workflows` → `200 { workflows: [{ id, name, path, state, htmlUrl, lastRun }], recentRuns: [{ ...run, workflowId }], degraded }` — last-run badges come from a single repo-level runs call grouped by `workflowId` (2 GitHub calls total, no N+1); `recentRuns` carries the repo's latest 5 runs for the page's "Latest runs" strip. A server-side deadline (`Taskboard:GitHub:WorkflowsDeadlineSeconds`, default 25s) races the call — on timeout/failure the endpoint still returns `200` with `degraded: true` and badge-less workflows instead of an error. `GET .../workflows/{id}/runs?take=10` → `200 { runs: [{ id, name, displayTitle, runNumber, event, status, conclusion, headBranch, headSha, actorLogin, createdAt, updatedAt, runStartedAt, htmlUrl, workflowId }] }`. Unknown repo → `404 { "error": "repo-not-found" }`. The UI badges map `success`→green, `failure`/`timed_out`/`startup_failure`/`action_required`→red, `in_progress`/`queued`/`requested`/`waiting`/`pending`→pulsing amber (drives a 60s auto-refresh), everything else→gray; failures show a Retry alert and keep the last valid list.
...
`GET /api/agents/events` (SPEC-20260921-agent-execution-event-pipeline) replays the normalized agent execution event stream persisted per scope — `scopeKind` is `run` (cockpit pipeline run), `thread` (AI Chat agent session) or `issue` (board one-shot run). Events carry `sequence` (monotonic per scope), `kind` (`lifecycle|message|thought|plan|tool_call|tool_output|permission|output|diff|verification|metric|error|approval|steer|activity|commands|session_info|terminal`), optional `stageId`/`sessionId`/`toolCallId`/`parentEventId`/`messageId`/`planId`/`patchOp` correlation, `payloadJson` (redacted + truncated) and `rawJson`. `after`/`take` paginate by sequence (`400` on invalid scope). Live events also stream over SignalR `/agent-log-hub` group `agent:{scopeKind}:{scopeId}` (`SubscribeToScope`, `ReceiveAgentEvent`). ACP sessions implement the real v1 protocol (SPEC-20260921-acp-v1-conformance): `initialize` with capability negotiation (`agentCapabilities`, `authMethods` → `authenticate` when required) → `session/new` (with `cwd`, `mcpServers` — the configured RAG/Knowledge MCP is injected — and `additionalDirectories` when supported) or `session/resume`/`session/load` after a reconnect → `session/prompt` content blocks. `session/cancel` is a notification and pending permission requests are answered `cancelled`; `session/request_permission` replies map `allow`/`deny`/`always` onto the agent's typed option kinds (`allow_once`/`allow_always`/`reject_once`/`reject_always`). Client-side `fs/read_text_file`/`fs/write_text_file` (sandboxed to the workspace, writes permission-gated) and `terminal/*` (opt-in via `Taskboard:Acp:ClientTerminal`) are served when advertised; undeclared methods get `-32601`. `Taskboard:Acp:SessionRuns=true` routes one-shot board/cockpit runs through the same session machinery; `Taskboard:Acp:TcpPort=N` connects to an already-running ACP server (e.g. `copilot --acp --port N`) instead of spawning a subprocess.

Protocol negotiation is version-aware (SPEC-20260921-acp-v2-readiness): the client offers `protocolVersion` up to `Taskboard:Acp:MaxProtocolVersion` (default `1`) and picks a per-connection `IAcpDialect` from the agent's answer — a v1-only agent transparently falls back to v1 even when `2` is offered, while answers above the offer fail with `unsupported_version`. ACP v2 is strictly opt-in while draft (`Taskboard:Acp:MaxProtocolVersion=2`): `session/prompt` returns an ack `messageId` and the turn closes on an `idle` `state_update` with `stopReason` (per-dialect `ITurnTracker`); `tool_call_update`, `agent_message*`/`plan_update` are upserts correlated by `toolCallId`/`messageId`/`planId` and events carry `patchOp` (`append`/`replace`/`clear`) so the timeline folds them into the same normalized model; `session/resume`+`replayFrom` replaces `session/load` and `session/list`/`session/delete` become baseline capabilities. The removed v1 client surface (`fs/*`, `terminal/*`, `session/load`, `session/set_mode`) is never served (`-32601`) nor sent on v2 connections — client tools run as MCP servers instead. NDJSON batch arrays are tolerated and unknown update variants/`_`-prefixed extensions are preserved raw for forward compatibility.

`POST /api/agents/control` (SPEC-20260921-board-cockpit-agent-observability) is the unified control surface — body `{ scopeKind, scopeId, action, stageId?, content?, configId? }`. Supported pairs: `issue`+`cancel`/`steer`/`retry` (board run — resolves to the issue's latest pipeline execution, falling back to the legacy one-shot orchestrator; `retry` targets `stageId` or the failed stage), `run`+`cancel`/`steer`/`retry` (cockpit pipeline; `retry` requires `stageId`, `steer` requires `content`), `thread`+`cancel`/`steer`/`set_config`/`set_mode` (AI Chat session; `set_config` requires `configId`+`content` as the option value — e.g. switch the model — and maps to ACP `session/set_config_option`, `set_mode` maps to `session/set_mode` for agents exposing `modes`). `400` unknown scope/action, `409` when the action is unsupported or the target has no active run/session, `202`/`200` on success. Every accepted action emits a normalized `lifecycle`/`steer` event into the scope stream.

`POST /api/agents/permissions/reply` body `{ scopeKind, scopeId, requestId, outcome, comment? }` routes permission replies by scope: `thread` → the ACP session permission gate (`410` when the request expired/unknown), `run` → the pipeline stage gate when `requestId` is `stage:{stageKey}` (`outcome` `deny` rejects, anything else approves), `issue` → the issue's pipeline stage gate for `stage:`-prefixed requestIds (`409` when no live run exists or the request is not a stage gate).

`GET /api/agents/state?scopeKind&scopeId` returns `{ scopeKind, scopeId, state, lastEventSequence, sessionId?, sessionInfoJson? }` — `run` resolves the pipeline status explicitly (`running|waiting_permission|awaiting_retry|paused|completed|stopped|queued`, `404` unknown run), `thread` reports `running`/`idle` from the live ACP session plus the negotiated peer snapshot (`sessionInfoJson`: `protocolVersion`, agent identity, `modes`, `configOptions`, `authMethods` — the agent's real model catalog lives in `configOptions` entries with `category:"model"`), `issue` resolves the issue's latest pipeline execution status (legacy `AgentRun` records as fallback). Used by the UI to rebuild the timeline after reconnect. Pipeline events of issue-bound runs are mirrored into both `run:{runId}` and `issue:{issueId}` scopes, so the Board task log and the Cockpit share the same normalized stream.
...
`POST` body `{ runId, repositoryPath, baseBranch?, taskSlug, retainOnFailure? }` → `201` with `{ worktreeId, runId, path, branch, status, repositoryPath, baseBranch, commitSha?, retainOnFailure, createdAt, updatedAt, version }`. Creates a dedicated `git worktree` under `{worktreeRoot}/{runId}` — `Taskboard:WorktreeRoot` (with `~` expansion), default `~/repos` so run worktrees sit next to the clones in the workspace — on a new `feature/agent-{runId}-{slug}` branch (incremental suffix on collision); `baseBranch` defaults to `main` in the API and to `HEAD` when the agent orchestrator isolates a run itself. Idempotent per `runId` — a live session is reused. An existing directory at the target is only cleaned when it is a stale worktree (`.git` file) or empty — anything else (e.g. a real clone) fails instead of being deleted.
...
`GET templates` → `200` with the four built-in DAG templates: `standard-feature` (architect → approval-gate → builder → verifier → reviewer), `quick-patch` (builder → verifier), `test-driven` (architect → tester → builder → verifier) and `single-agent` (builder → verifier). Each template also returns `stages: [{ key, name, kind, defaultAgent?, defaultTier?, dependsOn }]` so the UI can render per-stage pickers.

`POST start` body `{ templateId, repositoryFullName, repositoryPath, baseBranch, issueId?, initialPrompt, maxBudgetUsd?, agentOverride?, tierOverride?, skipVerification?, stageOverrides?, singleAgent?, singleAgentType?, singleAgentTier? }` → `201` with `{ pipelineExecutionId, templateId, status, worktreePath?, repositoryPath?, failureReason?, stages: [{ stageKey, name, kind, status, role?, agent?, attempts, handoffSummary?, lastError?, dependsOn, triedAgents?, autoRetryCount?, nextAutoRetryAtUtc? }] }`. Unknown template or a cyclic stage graph → `400 INVALID_PIPELINE_DAG`. `agentOverride`/`tierOverride`/`skipVerification` are `single-agent`-only — they replace the AgentWork stage's agent/tier and drop the Verification stage; sending them with any other template → `400`.

SPEC-20260923-cockpit-run-hardening RF-001: `repositoryPath` in the body is ignored — the clone is provisioned server-side by `IRepositoryProvisioningService` as `<workspace root>/<repo-name>` (`git clone` when missing/empty; an existing directory is reused only when it is a git work tree whose `origin` matches `owner/name`; foreign repos and non-repo dirs → `422 REPOSITORY_PROVISIONING_FAILED`, never deleted or silently reused). The returned `repositoryPath` is the provisioned clone and the run worktree is always cut from it — the workspace root itself is never the repository path.

`stageOverrides` is a map `stageKey → { agent?, tier? }` valid for every template's AgentWork stages (unknown keys, overrides on non-agent stages or `null` agent+tier → `400`). `singleAgent: true` binds one CLI to every AgentWork stage — `singleAgentType` picks it explicitly, `null` means Auto (first eligible). At start every AgentWork stage resolves its CLI against the eligible set (installed + authenticated + enabled via `IAgentEligibilityService`): template default when eligible, otherwise Auto; an explicitly picked but ineligible CLI → `422 AGENT_NOT_ELIGIBLE`, and no eligible CLI at all → `422` as well.
...
`POST .../approve` body `{ comment? }` releases a `WaitingApproval` gate and dispatches dependents → `200`. `POST .../retry` body `{ adjustedPrompt? }` re-queues a `Failed` stage without restarting the pipeline → `202` (clears `triedAgents` so the full fallback chain is available again). `POST .../cancel` stops in-flight stage processes and transitions the pipeline to `Cancelled` → `200`. Approving/retrying a stage in a non-matching state → `400`; unknown pipeline → `404`.

Dispatch runs on the `PipelineEngineService` background timer plus synchronous kicks after `start`/`approve`/`retry`; `PipelineEngine` resolves the DAG per execution scope so parallel stages never share a `DbContext`. Each AgentWork dispatch runs the stage's bound CLI; when the emitted model is rejected (`unrecognized_model`) the engine retries once on the same CLI without a model flag. Usage/cost are attributed to the CLI that actually ran each attempt and every billed attempt emits a `metric` event.

SPEC-20260923 RF-002 supersedes the same-tick fallback: a failed stage in an `AwaitingRetry` execution is auto-retried on a persisted schedule (`autoRetryCount`/`nextAutoRetryAtUtc` survive restarts) — `Taskboard:Pipelines:AutoRetry` (`Enabled` default `true`, `AttemptsPerAgent` default 5, `Interval` default `00:01:00`). After the bound CLI exhausts its attempts the sweep rotates to the next eligible CLI not in `triedAgents` and retries the same budget; a bound CLI that became ineligible rotates immediately. When no eligible untried CLI remains the run ends in terminal `Failed` with `failureReason` (stage key, total attempts, tried agents, last error) — stages still pending/waiting become `Skipped` and nothing new is dispatched. Retry scheduling, dispatch, rotation and the terminal failure are emitted as `stage`/`error`/`run_status` events; the run page surfaces `failureReason` in a danger alert.
...
Cockpit "runs" are pipeline executions. `GET runs` → `200` with recent executions (newest first, same shape as `GET /api/harness/pipelines/{id}`). `POST runs` body `{ templateId, repositoryFullName, baseBranch, issueId?, specPath?, prompt, maxBudgetUsd?, agentOverride?, tierOverride?, skipVerification?, stageOverrides?, singleAgent?, singleAgentType?, singleAgentTier? }` sends no repository path — the clone is provisioned server-side by `IRepositoryProvisioningService` (`~/repos/<name>`, confined to the workspace root, foreign/non-repo dirs → `422`), appends `specPath` content to the prompt when readable, and starts the pipeline → `201`. The overrides follow the same rules as `POST pipelines/start` — `singleAgent`/`stageOverrides` apply to any template, `agentOverride`/`tierOverride`/`skipVerification` remain `single-agent`-only — and ineligible explicit picks return `422`. When `issueId` is present a `pipeline-run` `IssueHistoryEvent` links the issue to the run id (issue History tab → "Open in Cockpit"). `GET {id}` → `200 { execution, telemetry?, worktree? }` or `404`.

`GET {id}/events` → `200 { events, nextAfter, hasMore }` (SPEC-20260923 RF-004): `events` are `CockpitEventDto` (`{ runId, timestampUtc, kind, title, payloadJson? }`) read from the durable normalized event stream (`IAgentExecutionEventSink` — survives restarts), merged with live-only `agent_output` chunks still in the volatile buffer. `after`/`take` paginate by event sequence (`after=nextAfter` for the next page). Normalized `output`/`lifecycle`/`verification`/`error`/`approval`/`steer`/`metric` kinds map back to cockpit kinds for replay — events also stream live over the SignalR hub below.
...
`POST {id}/approvals/{requestId}` body `{ action, comment? }` — `requestId` uses the `stage:<stageKey>` convention published by `RequireApproval`; `Allow` approves the `WaitingApproval` stage, `Deny` rejects it with the comment as the reason and ends the run in terminal `Failed` → `200`. Unknown run/request → `404`.
...
SignalR hub `/harness-cockpit-hub` (authenticated): client → server `JoinRunGroup(runId)` / `LeaveRunGroup(runId)` / `JoinRunsGroup` / `LeaveRunsGroup` — the shared `runs` group carries every run's events so the list page updates live; server → client `ReceiveCockpitEvent(CockpitEventDto)` and `RequireApproval(ApprovalRequestDto)` (`{ runId, requestId, title, description, options, summary? }` — `summary` is a one-line repo · run · stage · handoff digest used by the toast/browser notification). Every execution-level transition publishes a `run_status` event (`{ status, completedAtUtc? }` payload) to both the run group and `runs`; list rows show a `Needs approval` badge while a gate waits. Approval pushes surface as an in-app toast plus a browser Notification (when permitted) before the modal opens. The Blazor pages are `/cockpit` (list + start, live status refresh + pending-approval count, reconnect reconciliation) and `/cockpit/runs/{id}` (live timeline, Logs terminal with follow-scroll + stage/stream filters + copy/download, interactive **Shell** tab — `TerminalHub.OpenForRun(runId)` opens a PTY whose cwd is the run worktree resolved server-side and confined to the workspace root — Arquivos explorer tab, per-file collapsible Diff tab, steer bar, approval modal, FinOps header).
...
`GET summary` → `200 { totalCostUsd, totalTokens, runsCount, costByAgent{}, costByModel{}, dailyCosts[], cliUsage?, tokenWindows?, modelUsage{}, activityBins[], recentSessions[], alerts[] }` aggregated from `RunCostMetrics` + `CliSessionMetrics`/`CliDailyUsageAggregates` (SQLite). `period` defaults to `last-30-days`; values outside the allowed set return `400`. The detail blocks (SPEC-20260922-finops-dashboard-detail) carry token windows anchored at `now`, `Source · Model` distribution, hour/day activity bins (24 hourly bins for ≤2d spans, ≤40 day bins otherwise), a merged top-10 sessions list with `running`/`finished` status derived from the `Taskboard:FinOps:ActiveWindowSeconds` window (default 1800), and `warn`/`crit` alerts. `cliUsage.estimatedShare` and per-row `estimated` flags mark token counts/costs that are estimated rather than vendor-reported; CLI models without a specific `ModelPriceRate` are costed at a flat `FinOpsPricing.FallbackUsdPerMTok` ($9.5/1M tok). `cliUsage.byCli[]` (SPEC-20260922-finops-cli-usage-breakdown) adds one row per CLI kind with in-period aggregates — `{ cli, sessions, tokensInput, tokensOutput, tokensCached, costUsd, estimatedShare }`, ordered `sessions` desc / `costUsd` desc / `cli` asc; kinds with all-zero tokens/cost are included. `costByCli{}` is kept unchanged for backward compatibility.
