...
> **Branding:** o nome do produto é **Harness** (título, menus, login, README) e o repositório é `agent-harness` (renomeado de `taskboard-ai`). Identificadores técnicos internos permanecem `taskboard` — namespaces `Taskboard.*`, projetos, CLI `taskctl` e a seção de config `Taskboard:*` NÃO são renomeados. Superfície operacional usa o nome do produto (SPEC-20260922-harness-home-rename): env vars `HARNESS_*` (legadas `TASKBOARD_*` com fallback depreciado), home `~/.agent-harness`, banco `harness.sqlite`, unit `harness-server.service`.
...
- `.github/workflows/dotnet.yml`: build/test, verificação de formatação, gate de cobertura (`COVERAGE_THRESHOLD` — ratchet: hoje 77%, sobe a cada sprint até ≥80%; meta 90%) e pacotes vulneráveis. Baseline medido: 77.85% (2026-09-22).
...
6. **Coverage**: gate ratchet — `COVERAGE_THRESHOLD` em `dotnet.yml` nunca desce e sobe a cada sprint até ≥80% (meta 90%). Hoje: 77%.
