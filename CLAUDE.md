...
- `.github/workflows/dotnet.yml`: build/test, verificação de formatação, gate de cobertura (`COVERAGE_THRESHOLD` — ratchet: hoje 73%, sobe a cada sprint até ≥80%; meta 90%) e pacotes vulneráveis. Baseline medido: 73.70% (2026-09-20).
...
6. **Coverage**: gate ratchet — `COVERAGE_THRESHOLD` em `dotnet.yml` nunca desce e sobe a cada sprint até ≥80% (meta 90%). Hoje: 73%.
